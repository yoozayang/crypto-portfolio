import {loadRemoteCollections,syncRemoteCollections} from './supabase-data.js?v=20260911-4';
...
function queueRemoteSync(state){
  if(sessionStorage.getItem('cryptoPortfolioEditUnlocked')!=='1')return;
  const fp=fingerprint(state);if(fp===remoteFingerprint)return;
  clearTimeout(syncTimer);
  const snapshot=clone(sharedSlice(state));
  syncTimer=setTimeout(async()=>{try{await syncRemoteCollections(snapshot);remoteFingerprint=JSON.stringify(snapshot);console.info('Portfolio shared data synced to Supabase.')}catch(err){console.error('Supabase sync failed.',err);alert('雲端同步失敗，本機快取仍保留；請重新整理後再試。')}},250);
}
export async function loadSeed(){
  const [data,labels,baseline,remote]=await Promise.all([
  ...
  data.strategySettlements=remote.strategySettlements;
  data.ledgerEntries=remote.ledgerEntries;
  remoteFingerprint=JSON.stringify(remote);
  data.dataVersion=Math.max(+data.dataVersion||1,12);
