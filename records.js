...
const normalizeSeq=state=>{
  state.nextEventSeq=Math.max(+state.nextEventSeq||1,maxSeq(state)+1);
  for(const t of state.transactions||[])if(!t.seq)t.seq=state.nextEventSeq++;
  for(const e of state.ledgerEntries||[])if(!e.seq)e.seq=state.nextEventSeq++;
  state.nextEventSeq=Math.max(state.nextEventSeq,maxSeq(state)+1);
  return state;
};
...
export async function loadSeed(){
  ...
  data.dataVersion=Math.max(+data.dataVersion||1,6);data.reconciliation=data.reconciliation||{};data.reconciliation.actualHoldings=data.reconciliation.actualHoldings||{};
