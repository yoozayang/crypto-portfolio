...
import {ledgerMetrics,currentActualHoldings} from './calculations.js?v=20260907-1400';
...
function ensureTab(){
  ...
  const btn=document.createElement('button');btn.dataset.tab='ledger';btn.textContent='資金帳本';
...
function deleteEntry(id){if(!isUnlocked()){askUnlock();return;}if(!confirm('刪除這筆資金帳本紀錄？'))return;const state=load(),e=(state.ledgerEntries||[]).find(x=>x.id===id);if(e?.linkedTransactionIds?.length)state.transactions=state.transactions.filter(t=>!e.linkedTransactionIds.includes(t.id));state.ledgerEntries=(state.ledgerEntries||[]).filter(x=>x.id!==id);persist(state);}
...
function renderCapitalStrip(){const host=document.querySelector('#dashboard');if(!host||host.classList.contains('hidden'))return;const old=host.querySelector('.capital-strip');old?.remove();const state=load(),m=ledgerMetrics(state),box=document.createElement('div');box.className='grid section capital-strip';box.innerHTML=`<div class="card kpi"><div class="label">淨投入本金</div><div class="value">${fmt(m.netCapital)} U</div></div><div class="card kpi"><div class="label">第一倉淨本金</div><div class="value">${fmt(m.byWarehouse.第一倉.net)} U</div></div><div class="card kpi"><div class="label">第二倉淨本金</div><div class="value">${fmt(m.byWarehouse.第二倉.net)} U</div></div><div class="card kpi"><div class="label">累積非交易收益</div><div class="value good">${fmt(m.income)} U</div></div>`;host.prepend(box);}
...
function renderLedger(editId=0){
  ...
  host.innerHTML=`<div class="grid"><div class="card kpi"><div class="label">第一倉淨本金</div><div class="value">${fmt(m.byWarehouse.第一倉.net)} U</div><div class="subtle">初始 ${fmt(m.byWarehouse.第一倉.opening)} · 入 ${fmt(m.byWarehouse.第一倉.deposits)} · 出 ${fmt(m.byWarehouse.第一倉.withdrawals)}</div></div><div class="card kpi"><div class="label">第二倉淨本金</div><div class="value">${fmt(m.byWarehouse.第二倉.net)} U</div><div class="subtle">初始 ${fmt(m.byWarehouse.第二倉.opening)} · 入 ${fmt(m.byWarehouse.第二倉.deposits)} · 出 ${fmt(m.byWarehouse.第二倉.withdrawals)}</div></div><div class="card kpi"><div class="label">非交易收益</div><div class="value good">${fmt(m.income)} U</div></div></div>
  <div class="card section"><div class="ledger-head"><h2>${edit?'修改紀錄':'新增資金紀錄'}</h2><span class="subtle">入出金改變本金；換倉與交易所轉帳不改變本金。</span></div><form id="ledgerForm" class="form-grid"><input type="hidden" name="editId" value="${edit?.id||''}">${select('type','類型',[{value:'deposit',label:'入金'},{value:'withdraw',label:'出金'},{value:'swap',label:'倉內換倉'},{value:'transfer',label:'交易所轉帳'},{value:'income',label:'收益 / 空投'}],type)}<div class="full ledger-fields">${fieldsFor(type,state,edit||{})}</div><label class="full"><span>備註</span><textarea name="note">${esc(edit?.note||'')}</textarea></label><div class="full toolbar"><button class="btn" type="button" id="ledgerClear">清空</button><button class="btn primary" type="submit">${edit?'儲存修改':'新增紀錄'}</button></div></form></div>
